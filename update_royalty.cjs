// Set royalty + creators on all CAPY Warriors via the program's authority-gated
// `update_royalty`, then verify the creator with Metaplex SignMetadata.
//
//   node update_royalty.cjs                         dry-run, mainnet, all 500
...
const CLUSTERS = {
  mainnet: { rpc: "https://rpc.mainnet.x1.xyz", program: "6r9HZKQRhDfNnZM4m6TgkcK82Bt6EA1q2Ck9VNWoTnGm" },
  testnet: { rpc: "https://rpc.testnet.x1.xyz", program: "9TjTjyiz3gpRrTaeGvxi2LTrjjsYmDers7VQVDxo9Zdh" },
};
const MPL = new PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s");

// ── Target state ───────────────────────────────────────────────────────
const TARGET_BPS = 100; // 1%
const DEFAULT_CREATOR = "GZuBHE3fQCQ6eSTLMwWKrK15CjtWfA58BmxdtWwG5mJJ"; // capy-lp-treasury
...
const AUTH_KP = opt("--authority-keypair", process.env.HOME + "/.config/solana/capy-mint-authority.json");
const CREATOR_KP = opt("--creator-keypair", process.env.HOME + "/.config/solana/capy-lp-treasury.json");
const CONCURRENCY = 4;
